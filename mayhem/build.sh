#!/usr/bin/env bash
#
# mayhem/build.sh — build the docstring_parser Atheris fuzz harness + its standalone
...
#      resolves entirely from it (pip --no-index --find-links). docstring_parser itself stays the
#      editable source tree ($SRC/docstring_parser, exposed via PYTHONPATH) so a PATCH agent's
#      edits take effect with no reinstall.
#   2. Compile launcher.c -> the ELF Mayhem target `docstring_parser_fuzzer` (Atheris is a Python
...
#      `docstring_parser_fuzzer-standalone`.
...
# docstring_parser is a flat package at the repo root ($SRC/docstring_parser); expose it (and the
# site dir) on PYTHONPATH so a PATCH agent's edits take effect immediately with no reinstall.
PYRUN="$SITE:$SRC"
...
PYTHONPATH="$PYRUN" "$PY" -c 'import atheris, pytest, docstring_parser; print("imports OK")'
...
#    run time (the Dockerfile sets ENV PYTHONPATH), so the Python side finds atheris + the library.
HARNESS="$SRC/mayhem/fuzz_str.py"
echo ">> compiling docstring_parser_fuzzer (+ standalone) with DEBUG_FLAGS=$DEBUG_FLAGS"
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/docstring_parser_fuzzer"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/docstring_parser_fuzzer-standalone"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" "$SRC/mayhem/run_tests.c" -o "$SRC/docstring_parser_run_tests"

echo ">> build.sh complete"
ls -la "$SRC/docstring_parser_fuzzer" "$SRC/docstring_parser_fuzzer-standalone" "$SRC/docstring_parser_run_tests"
